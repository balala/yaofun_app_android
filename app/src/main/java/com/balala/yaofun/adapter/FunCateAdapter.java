...
public class FunCateAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private ArrayList<BaseBean<FunhomeData.DataBean.FoodBean>> list;

    public FunCateAdapter(Context context, ArrayList<BaseBean<FunhomeData.DataBean.FoodBean>> list) {
        this.context = context;
        this.list = list;
    }

    public void setList(ArrayList<BaseBean<FunhomeData.DataBean.FoodBean>> list) {
        this.list = list;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        BaseBean<FunhomeData.DataBean.FoodBean> data = list.get(position);
        if (holder instanceof FunCateAdapter.ViewHolderA) {
            FunCateAdapter.ViewHolderA viewHolderA = (FunCateAdapter.ViewHolderA) holder;
            Glide.with(context).load(data.getData().getUser_images()).into(viewHolderA.img_a);
            viewHolderA.tv_a1.setText(data.getData().getGroup_name());
            viewHolderA.tv_a2.setText(data.getData().getAutograph());
            ...
            Glide.with(context).load(data.getData().getUser_images()).into(viewHolderB.img_b);
            viewHolderB.tv_b1.setText(data.getData().getGroup_name());
            viewHolderB.tv_b2.setText(data.getData().getAutograph());
        }
